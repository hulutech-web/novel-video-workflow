...
func main() {
	...
	fmt.Println("🚀 启动智能分章节+生音频+生台词功能测试...")
	
	// 检查所需服务是否可用
	fmt.Println("🔍 检查所需服务可用性...")
	if err := checkServicesAvailability(logger); err != nil {
		fmt.Printf("❌ 服务可用性检查失败: %v\n", err)
		return
	}
	fmt.Println("✅ 所有服务都可用")
	...
	fmt.Println("✅ 智能分章节+生音频+生台词功能测试完成！")
}

// checkServicesAvailability 检查所有必需的服务是否可用
func checkServicesAvailability(logger *zap.Logger) error {
	// 1. 检查 Ollama 服务
	fmt.Println("  📡 检查 Ollama 服务...")
	resp, err := http.Get("http://localhost:11434/api/tags")
	if err != nil || resp.StatusCode != http.StatusOK {
		if resp != nil {
			resp.Body.Close()
		}
		return fmt.Errorf("Ollama 服务不可用: %v", err)
	}
	if resp != nil {
		resp.Body.Close()
	}
	fmt.Println("  ✅ Ollama 服务可用")

	// 2. 检查 DrawThings 服务
	fmt.Println("  🎨 检查 DrawThings 服务...")
	drawClient := drawthings.NewDrawThingsClient(logger, "http://localhost:7861")
	if !drawClient.APIAvailable {
		return fmt.Errorf("DrawThings 服务不可用")
	}
	fmt.Println("  ✅ DrawThings 服务可用")

	// 3. 检查 IndexTTS2 服务 (可选，因为可能未启动)
	fmt.Println("  🗣️  检查 IndexTTS2 服务...")
	resp, err = http.Get("http://localhost:7860")
	if err != nil {
		fmt.Printf("  ⚠️  IndexTTS2 服务不可用，音频生成功能将被跳过: %v\n", err)
	} else {
		resp.Body.Close()
		fmt.Println("  ✅ IndexTTS2 服务可用")
	}

	// 4. 检查 Aegisub (这是一个脚本，不是服务，但我们可以检查脚本是否存在)
	fmt.Println("  📄 检查 Aegisub 脚本...")
	aegisubGen := aegisub.NewAegisubGenerator()
	if _, err := os.Stat(aegisubGen.ScriptPath); os.IsNotExist(err) {
		fmt.Printf("  ⚠️  Aegisub 脚本不存在，字幕生成功能将被跳过: %v\n", err)
	} else {
		fmt.Println("  ✅ Aegisub 脚本可用")
	}
...
func (wp *WorkflowProcessor) processNovelDirectory(novelDir string) error {
	...
	for _, entry := range entries {
		if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".txt") {
			novelFilePath = filepath.Join(novelDir, entry.Name())
			break
		}
	}
	...
	if novelFilePath != "" {
		// 获取小说文件的修改时间
		novelFileInfo, err := os.Stat(novelFilePath)
		if err == nil {
			// 检查现有的章节文件中最新的修改时间
			for _, entry := range entries {
				if entry.IsDir() && strings.HasPrefix(entry.Name(), "chapter_") {
					chapterDir := filepath.Join(novelDir, entry.Name())
					chapterEntries, _ := ioutil.ReadDir(chapterDir)
					for _, chapterEntry := range chapterEntries {
						if strings.HasSuffix(chapterEntry.Name(), ".txt") {
							chapterFile := filepath.Join(chapterDir, chapterEntry.Name())
							chapterFileInfo, err := os.Stat(chapterFile)
							if err == nil {
								// 如果小说文件比章节文件新，则需要重新拆分
								if novelFileInfo.ModTime().After(chapterFileInfo.ModTime()) {
									shouldResplit = true
									break
								}
							}
	...
	// 如果小说文件比现有章节文件新，则重新拆分
	if novelFilePath != "" && shouldResplit {
		fmt.Printf("🔄 检测到小说文件已更新，重新拆分章节...\n")
		
		// 创建FileManager实例进行章节拆分
		fm := file.NewFileManager()
		createdFiles, err := fm.SplitNovelFileIntoChapters(novelFilePath)
		if err != nil {
			return fmt.Errorf("自动拆分小说章节失败: %v", err)
		}
		
		fmt.Printf("✅ 自动章节拆分完成！创建了 %d 个章节文件\n", len(createdFiles))
	...
	// 如果在子目录中没有找到文件，检查是否有完整的小说文件需要拆分
	if len(files) == 0 {
		// 查找根目录下的.txt文件
		for _, entry := range entries {
			if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".txt") {
				fullNovelPath := filepath.Join(novelDir, entry.Name())
				fmt.Printf("📚 检测到完整小说文件: %s，开始自动拆分章节...\n", fullNovelPath)
				
				// 创建FileManager实例进行章节拆分
				fm := file.NewFileManager()
				createdFiles, err := fm.SplitNovelFileIntoChapters(fullNovelPath)
				if err != nil {
					return fmt.Errorf("自动拆分小说章节失败: %v", err)
				}
		...
		return fmt.Errorf("在目录 %s 的chapter_XX子目录中未找到任何文本文件，也没有找到可用于拆分的小说文件", novelDir)
	}
...
func (wp *WorkflowProcessor) processSingleTextFile(txtFile string) error {
	...
	if novelName == "input" {
		...
		// 对于直接放在input目录下的完整小说文件，需要先进行章节拆分
		fmt.Printf("📚 检测到完整小说文件: %s，开始自动拆分章节...\n", txtFile)
		
		// 创建FileManager实例进行章节拆分
		fm := file.NewFileManager()
		createdFiles, err := fm.SplitNovelFileIntoChapters(txtFile)
		if err != nil {
			return fmt.Errorf("自动拆分小说章节失败: %v", err)
		}
		...
		// 章节拆分完成后，处理新创建的章节文件
		for _, chapterFile := range createdFiles {
			err := wp.processSingleTextFileWithNovelName(chapterFile, novelName)
			if err != nil {
				wp.logger.Error("处理拆分后的章节文件失败", zap.String("file", chapterFile), zap.Error(err))
				continue
			}
		}
		return nil
...
func (wp *WorkflowProcessor) extractChapterNumberFromPath(filePath string) int {
	...
	// 如果都没找到，尝试从完整路径中查找chapter_xx模式
	// 遍历路径的每个部分
	pathParts := strings.Split(filePath, string(os.PathSeparator))
	for _, part := range pathParts {
		if strings.HasPrefix(part, "chapter_") {
			numStr := strings.TrimPrefix(part, "chapter_")
			numStr = strings.TrimPrefix(numStr, "_")

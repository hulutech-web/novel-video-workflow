...
本文档描述了小说视频生成的完整工作流程，包括智能分章节、音频生成、台词生成和图像生成。

## 功能特点

1. **智能分章节**：自动识别小说章节边界，将长篇小说拆分为独立章节
2. **音频生成**：使用IndexTTS2将文本转换为语音
3. **台词生成**：基于音频和文本生成同步字幕
4. **图像生成**：使用AI模型根据文本内容生成匹配图像
5. **高效处理**：图像尺寸优化为512x896以提高生成效率
6. **文件组织**：所有输出文件按章节分别存储在独立文件夹中
7. **智能分镜**：使用Ollama分析整个章节内容并生成分镜提示词
...
3. 对每个章节依次执行：
   - 音频生成（使用参考音频进行TTS）
   - 台词/字幕生成（基于音频和文本）
   - 图像生成（使用Ollama分析内容并生成分镜图像）
4. 所有输出文件按章节分别存储

## 分镜图像生成

新的工作流程使用Ollama来分析整个章节内容：
- Ollama整体读取章节内容（chapter_0x.txt）
- 分析内容并处理分镜
- 为每个分镜生成专门的提示词
...
- IndexTTS2服务运行在 `http://localhost:7860`
- DrawThings服务运行在 `http://localhost:7861`
- Ollama服务运行在 `http://localhost:11434`
- 图像生成使用 `z_image_turbo_1.0_q6p.ckpt` 模型
- 图像尺寸为 512x896 以提高生成效率
